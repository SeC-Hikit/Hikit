...
import org.sc.common.rest.TrailUpgradeDto
...
@Component
class TrailUpgradeValidator @Autowired constructor (
    private val coordsValidatorTrail: TrailCoordinatesValidator,
    private val placeRefValidator: PlaceRefValidator): Validator<TrailUpgradeDto> {
    ...
    // TODO
    override fun validate(request: TrailUpgradeDto): Set<String> {
        val errors = mutableSetOf<String>()
        if (isEmpty(request.id)) {
            errors.add(String.format(emptyFieldError, "Id"))
        }
