...
public class TrailMapper implements Mapper<Trail>, SelectiveArgumentMapper<Trail> {
    ...
    public Document mapToDocument(final Trail object) {
        ...
        return new Document()
                ...
                .append(Trail.COORDINATES_LOW, object.getCoordinates().stream()
                        .map(trailCoordinatesMapper::mapToDocument).collect(toList()))
                .append(Trail.COORDINATES_MEDIUM, object.getCoordinates().stream()
                        .map(trailCoordinatesMapper::mapToDocument).collect(toList()))
                .append(Trail.COORDINATES_HIGH, object.getCoordinates().stream()
                        .map(trailCoordinatesMapper::mapToDocument).collect(toList()))
